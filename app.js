document.addEventListener('DOMContentLoaded', () => {
  ...
  window.setVisualizerStyle = function(style) {
    ...
    const labels = { roller:'Roller Blinds', vertical:'Vertical Blinds', venetian:'Venetian Blinds', roman:'Roman Blinds', vision:'Vision (Zebra) Blinds', pleated:'Pleated Blinds', shutters:'Plantation Shutters' };
    choiceStyle.textContent = labels[style] || style;
    const allTabs = ['styleTabRoller','styleTabVertical','styleTabVenetian','styleTabRoman','styleTabVision','styleTabPleated','styleTabShutters'];
    allTabs.forEach(id => { const b = document.getElementById(id); if (b) b.classList.remove('active'); });
    const activeTabId = 'styleTab' + style.charAt(0).toUpperCase() + style.slice(1);
  ...
  window.setVisualizerColor = function(colorId, colorName, colorHex, isLight, colorDesc) {
    activeColorId = colorId; activeColorName = colorName; activeColorHex = colorHex;
    ...
    choiceColor.style.color = isLight ? '#475569' : colorHex;
    matBadge.style.backgroundColor = colorHex;
    matTitle.innerHTML = '<span style="width:14px;height:14px;border-radius:50%;display:inline-block;border:1px solid rgba(0,0,0,0.1);background:' + colorHex + ';margin-right:8px;"></span> ' + colorName + ' Fabric';
    matDesc.textContent = colorDesc;
    document.querySelectorAll('.swatch-item').forEach(btn => btn.classList.remove('active'));
    const evt = window.event;
    if (evt && evt.currentTarget) {
      evt.currentTarget.classList.add('active');
    } else {
      document.querySelectorAll('.swatch-item').forEach(btn => { if (btn.getAttribute('title') === colorName) btn.classList.add('active'); });
    }
  };
  ...
  window.adjustVisualizerHeight = function(height) {
    activeHeight = height;
    const fabric = document.getElementById('visualizerBlindFabric');
    const lbl = document.getElementById('visualizerHeightLabel');
    if (fabric && lbl) { fabric.style.height = height + '%'; lbl.textContent = height; }
  };
  ...
  window.applyVisualizerToBooking = function() {
    ...
    const styleMap = { roller:'Roller Blinds', vertical:'Vertical Blinds', venetian:'Venetian Blinds', roman:'Roman Blinds', vision:'Vision Blinds', pleated:'Pleated Blinds', shutters:'Window Shutters' };
    blindsSelect.value = styleMap[activeStyle] || 'Roller Blinds';
    const n = activeColorName.toLowerCase();
    if (n.includes('white')) colorSelect.value = 'White';
    else if (n.includes('cream') || n.includes('linen') || n.includes('sand')) colorSelect.value = 'Cream';
    else if (n.includes('grey') || n.includes('gray') || n.includes('charcoal') || n.includes('black')) colorSelect.value = n.includes('charcoal') ? 'Charcoal' : n.includes('black') ? 'Black' : 'Grey';
    else if (n.includes('oak') || n.includes('walnut')) colorSelect.value = 'Natural Wood';
    else if (n.includes('mustard') || n.includes('terracotta') || n.includes('navy') || n.includes('mauve')) colorSelect.value = 'Bright Colors';
    else if (n.includes('sage') || n.includes('blue') || n.includes('beige')) colorSelect.value = 'Patterned / Textured';
    blindsSelect.dispatchEvent(new Event('change'));
    colorSelect.dispatchEvent(new Event('change'));
    if (messageTextarea) {
      messageTextarea.value = 'Visualizer choice: ' + (styleMap[activeStyle] || activeStyle) + ' in "' + activeColorName + '" at ' + activeHeight + '% height.';
