...
3. Set in `.env`:
   ```
   EMAIL_USER=info@blissfulblindsltd.co.uk
...
The SQLite file lives at `data/reviews.db` (overridable via `DATA_DIR`).
Back it up regularly — it's the entire database. `data/` is gitignored on
purpose; don't commit it.

### Deploying to Render (recommended — matches `render.yaml` in this folder)

The main static site should NOT be deployed here — it goes to Vercel (or
any static host) separately. This is just the API + admin panel.

1. Push this repo to GitHub (already done if you're reading this from the repo).
2. In Render: **New → Blueprint**, select this repo. Render finds
   `review-system/render.yaml` automatically; set the Blueprint's root
...
3. Render will prompt for the secrets marked `sync: false` in the
   ...
   - `CORS_ORIGINS` — your real Vercel domain(s), e.g.
     `https://blissfulblindsltd.co.uk,https://blissfulblindsltd.co.uk`
...
5. **Free tier has no persistent disk** — the blueprint's `disk:` block
   needs a paid instance type (a few $/month) to actually take effect;
   on free, the SQLite database resets on every redeploy. Fine for
   testing, upgrade the instance type before relying on real data.
6. After first deploy, run once (Render's Shell tab, or a one-off job):
   ```bash
   node src/db/seed.js
